...
async function resolveToPlayable(query, requestedBy) {
  // 1) Si es Spotify link: convertir a búsqueda y resolver en YouTube
  if (query.includes('open.spotify.com')) {
    const sp = await play.spotify(query).catch(() => null);
    if (!sp) return null;

    // Puede ser track/album/playlist. Para empezar: track o primer track.
    const firstTrack = sp?.type === 'track'
      ? sp
      : (sp?.tracks?.[0] ?? null);
  ...
  if (query.startsWith('http://') || query.startsWith('https://')) {
    ...
    // YouTube directo (o links que play-dl pueda streamear)
    ...
    // link directo mp3 (puede funcionar, depende)
    return { title: 'Audio', url: query, thumbnail: null, requestedBy, sourceLabel: 'Direct' };
  }

  // 3) Búsqueda normal: YouTube primero, luego SoundCloud
  ...
  return {
    ...
    requestedBy,
    sourceLabel: r.url?.includes('soundcloud.com') ? 'SoundCloud' : 'YouTube',
  };
}
...
async function playNext(interaction, guildId) {
  ...
  let stream;
  try {
    stream = await play.stream(next.url); // real stream: YouTube/SoundCloud :contentReference[oaicite:5]{index=5}
  } catch (e) {
  ...
  const msg = await interaction.followUp({ embeds: [embed], components: [controlsRow(q)] }).catch(() => null);
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'play') {
    try {
      ...
      // Discord exige ack en 3s: deferReply :contentReference[oaicite:6]{index=6}
      ...
      if (!song) return interaction.editReply('❌ No encontré resultados. Prueba con URL o otro término.');
